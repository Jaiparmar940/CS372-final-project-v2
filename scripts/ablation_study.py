...
def run_ablation_study():
    ...
    reward_config = RewardConfig(
        landing_bonus=100.0,
        crash_penalty=-100.0,
        fuel_penalty_coef=0.1,
        smoothness_penalty_coef=0.05
    )
    ...
    components = ["Replay Buffer", "Target Network", "Custom Reward"]
    baseline_val = val_returns[0]
    component_impact = [
        val_returns[0] - val_returns[1],  # Impact of replay buffer
        val_returns[0] - val_returns[2],  # Impact of target network
        val_returns[3] - val_returns[0]    # Impact of custom reward (vs baseline)
    ]
